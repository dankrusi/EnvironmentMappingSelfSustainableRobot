...
Controller::Controller(QString name, Core *core, int interval) : CoreThread(name, core) {
	...
	// Watchdog this?
	if(core->watchdog && core->boolSetting("Controller_Watchdog_Enabled")) {
		watchdogAction = new ThreadMonitorWatchdogAction(QString("%1 Controller").arg(name), interval, core->doubleSetting("Controller_Watchdog_Flexibility"), core);
...
void Controller::stop() {
	...
	stopRequested = true;
	while(this->isRunning() == true) SleeperThread::msleep(core->intSetting("Controller_StopRequestInterval"));
...
void Controller::_emergencyStop() {
