 ...
 *  					such as 'COM4'.
 */
COIL::COIL(QString portName, Core *core) : CoreObject("COIL", core)
{
...
COIL::COIL(Core *core) : CoreObject("COIL", core)
{
	this->port = NULL;
}
 ...
 *	This method automatically allocates the correct memory for the caller,
 *	but the caller is responsible to cleanup...
 *
 *	\return		Pointer to array of sensor data or a NULL pointer on error.
 */
int* COIL::getAllSensors() {
	int* data = (int*)malloc (36*sizeof(int));
	return getAllSensors(data);
}
 ...
 *	\return		Pointer to array of sensor data or a NULL pointer on error.
 */
int* COIL::getAllSensors(int* result)
{
	byte buf[52];
	int i, numread;
