...
HeatMap::HeatMap(Core *core, long width, long height) : ColorMap(core, width, height)
{
	this->name = "Heat";
	distance = 2.0;
...
void HeatMap::registerHeat(ColorMap::MapChannel channel, long x, long y) {
	registerHeat(channel, x, y, core->doubleSetting("Map_HeatMap_HeatOpacity"), core->longSetting("Map_HeatMap_HeatSize"));
}
...
void HeatMap::registerHeat(ColorMap::MapChannel channel, long x, long y, double opacity, long size) {
	...
	//	// Only register heat if this exact spot has not already been registered before with this given heat type...
	//	if(getChannelHeat(HeatMap::MarkerChannel, x, y) == 0.0) {
	//
	//		// Init painter
	...
	//
	//		// Draw heat spot
	//		if(type == Core::AreaTypeCollision)		painter.setBrush(QBrush(QColor(core->stringSetting("Map_HeatMap_CollisionAreaColor"))));
	//		else if(type == Core::AreaTypeOpen)		painter.setBrush(QBrush(QColor(core->stringSetting("Map_HeatMap_OpenAreaColor"))));
	//		painter.drawEllipse(QRectF(x - (size/2), heat->height() - y - (size/2), size, size));
	//
	//		// Draw marker
	//		painter.setOpacity(1.0);
	//		painter.setPen(Qt::blue);
	//		painter.setBrush(QBrush(Qt::blue));
	//		painter.drawPoint(x, heat->height() - y);
	//	}
		...
		if     (channel == ColorMap::CollisionAreaChannel  && distance < dist(lastCollisionAreaPosition, Vector2D(x,y))){
			lastCollisionAreaPosition = Vector2D(x,y);
			painter.setBrush(QBrush(QColor(core->stringSetting("Map_HeatMap_CollisionAreaColor"))));
		}
		else if(channel == ColorMap::OpenAreaChannel && distance < dist(lastOpenAreaPosition, Vector2D(x,y))){
			painter.setBrush(QBrush(QColor(core->stringSetting("Map_HeatMap_OpenAreaColor"))));
			lastOpenAreaPosition = Vector2D(x,y);
		}
...
void HeatMap::registerHeat(ColorMap::MapChannel channel, Trafo2D position, double distance, double angle, double opacity, int size) {
	...
	Trafo2D objectLocation = position * Trafo2D::rot(Rad(angle)) * Trafo2D::trans(0, distance); // Core/COIL has a clockwise positive rotation while the math library has a anti-clockwise rotation...
...
void HeatMap::registerCollisionAtTrackerLocation(double distance, double angle, double opacity, int size) {
	registerHeat(ColorMap::CollisionAreaChannel, core->tracker->getTransformation(), distance, angle, opacity, size);
}
